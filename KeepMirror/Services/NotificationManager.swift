...
/// Manages user-facing notifications for KeepMirror.
...
/// KeepMirror targets **macOS 13**, so all options used here are fully available.
///
...
final class NotificationManager: NSObject, UNUserNotificationCenterDelegate {
    ...
    nonisolated static let expiryWarningNotifID = "com.keepmirror.expiry-warning"
    ...
    func notifyExpiryWarning(duration: ActivationDuration) {
        ...
        content.title = "KeepMirror — Session Ending Soon"
    ...
    func notifyAutoStop(reason: StopReason) {
        ...
        let content = UNMutableNotificationContent()
        content.title = "KeepMirror Stopped"
        ...
        let request = UNNotificationRequest(
            identifier: "com.keepmirror.autostop.\(UUID().uuidString)",
            content: content,
            trigger: nil
        )
        UNUserNotificationCenter.current().add(request)
