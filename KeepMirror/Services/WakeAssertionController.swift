...
/// Manages IOKit power assertions for KeepMirror sessions.
///
/// ## Assertion types
///
...
final class LiveWakeAssertionController: WakeAssertionControlling {
    ...
    func activate(allowDisplaySleep: Bool, allowPowerNap: Bool) throws {
        ...
        let systemName = allowPowerNap
            ? "KeepMirror — preventing sleep (Power Nap allowed)" as CFString
            : "KeepMirror — preventing idle system sleep" as CFString
        ...
        if !allowDisplaySleep {
            try createAssertion(
                ...
                name: "KeepMirror — keeping display on" as CFString,
